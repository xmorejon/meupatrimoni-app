...
function CallbackProcessor() {
  ...
  useEffect(() => {
    // Don't run this logic on the server or while auth is loading.
    ...
    // This logic should only run once after loading is complete.
        ...
        .then((result) => {
          if (result.success) {
            toast({
              ...
              description: `S'han importat correctament els comptes.`,
            });
          } else {
            toast({
              variant: "destructive",
              ...
              description:
                result.error ||
                "No s'ha pogut connectar el teu compte bancari.",
            });
          }
        })
        .finally(() => {
          router.replace("/"); // Always redirect after processing
        });
    } else if (error) {
  ...
  }, [loading, user, exchangeCode, router, toast]);
