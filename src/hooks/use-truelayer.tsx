...
export const useTrueLayer = () => {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "useTrueLayer.connect: Token refreshed, calling getAuthLink function.",
      );
      const result = await getAuthLink();
      console.log(
        "useTrueLayer.connect: getAuthLink function returned.",
        result,
      );
  ...
  const exchangeCode = useCallback(
    async (code: string, origin: string): Promise<ExchangeResult> => {
      ...
      try {
        ...
        console.log(
          "useTrueLayer.exchangeCode: User authenticated, UID:",
          user.uid,
        );

        console.log(
          "useTrueLayer.exchangeCode: Forcing token refresh before backend call.",
        );
        await user.getIdToken(true);
        console.log(
          "useTrueLayer.exchangeCode: Token refreshed. Calling handleCallback function now.",
        );

        const result = await handleCallback({ code, origin });

        console.log(
          "useTrueLayer.exchangeCode: handleCallback function returned a result.",
          result,
        );
        ...
        if (!data.success) {
          console.error(
            "useTrueLayer.exchangeCode: Backend operation was not successful.",
            data,
          );
        ...
        console.log(
          "useTrueLayer.exchangeCode: Finished, setting isLoading to false.",
        );
        setIsLoading(false);
      }
    },
